...
EXP_CONDITION = 'MSR' # reproduce (recursion/multiple calls possible), 'no_rec', 'delete_state' ...
PROBABILITIES = {
    'create_method': 0.1,
    'call_method': 0.4,
    'update_method': 0.45,
...
def run_repo_model():
    ...
    for sim in range(simulations):
        ...
        model = Evolver(iterations, FITNESS_METHOD, PROBABILITIES, EXP_CONDITION, add_prob, LOGGING)
        # model = Evolver()
...
def create_outputfile(iterations, add_prob):
    ...
    # Create file
    filename = 'results/result_msr_' + str(EXP_CONDITION) + '_fit' + str(FITNESS_METHOD) + '_its' + str(iterations) + '_addprob' + str(add_prob) + '_time' + str(datetime.datetime.now().strftime("%d_%H_%M_%S")) + '.csv'

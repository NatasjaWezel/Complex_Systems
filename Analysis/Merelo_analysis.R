...
for (i in 1:length(lines) ) {
  ...
  ggplot()+geom_point(data=sorted.lines,aes(x=x,y=Lines.changed))+scale_y_log10()+ggtitle(lines[[i]]$url)+
    theme(
      legend.position="none", 
      plot.title = element_text(color="white"),
      axis.title.x = element_text(color="white"),
...
for (i in 1:length(lines) ) {
  autoplot(pacf(lines[[i]]$Lines.changed, plot=FALSE,xlab="some name")) + theme(
    legend.position="none", 
    plot.title = element_text(color="white"),
    axis.title.x = element_text(color="white"),
    axis.title.y = element_text(color="white"),
